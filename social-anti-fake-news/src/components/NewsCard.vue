<template>
  ...
  >
    ...
    />
    <h2 class="text-lg md:text-xl font-semibold mb-1 text-yellow-500">{{ news.title }}</h2>
    <p class="text-sm md:text-base text-gray-200 flex-1">{{ news.shortDetail }}</p>
    <div class="mt-2 text-xs md:text-sm text-gray-400 flex justify-between">
      <span>By {{ news.reporter }}</span>
      <span>{{ formatDate(news.dateTime) }}</span>
    </div>
    <span 
      :class="['mt-2 inline-block px-2 py-1 text-xs rounded font-semibold', 
      news.status === 'Fake' ? 'bg-red-200 text-red-800' : 'bg-green-200 text-green-800']"
    >
      {{ news.status }}
    </span>
  </div>
