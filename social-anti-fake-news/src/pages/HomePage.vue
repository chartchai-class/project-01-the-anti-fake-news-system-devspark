<template>
  <div class="p-6 max-w-6xl mx-auto">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
      <div class="flex gap-2">
        <button v-for="option in filterOptions" :key="option.value" @click="setFilter(option.value)"
          :class="['px-4 py-2 rounded-lg text-sm font-medium transition shadow', filter===option.value ? 'bg-blue-600 text-white shadow-md':'bg-gray-100 hover:bg-gray-200']">
          {{ option.label }}
        </button>
      </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
      <div v-for="news in paginatedNews" :key="news.id" @click="goToDetail(news.id)"
